...
- 持久化重试、进度、日志和文件状态
- 定期刷新 rclone 挂载服务，但仅在没有活动下载和排队任务时才会执行重启
...
1. 先检查下载队列和活动下载是否都为空
2. 只要还有下载任务或排队任务，就直接跳过本轮刷新
3. 确认空闲后再暂停扫描
4. 重启配置中的 rclone systemd 服务
...
保存 `/api/config` 时，只有在守护进程报告“活动下载数为 0 且排队数为 0”时才会自动尝试执行 `systemctl restart sync.service`；否则只保存配置，不自动重启。
